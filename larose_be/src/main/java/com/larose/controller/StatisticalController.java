...
public class StatisticalController {
    ...
    @GetMapping("/revenue")
    public ResponseEntity<BigDecimal> sumTotalPrice(@RequestParam(required = false) Integer days) {
        return ResponseEntity.ok(statisticalService.sumTotalPrice(days));
    }

    @GetMapping("/revenue/daily")
    public ResponseEntity<List<DailyRevenueDto>> getDailyRevenue(@RequestParam(required = false) Integer days) {
        return ResponseEntity.ok(statisticalService.getDailyRevenue(days));
    }
