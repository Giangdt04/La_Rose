...
class UserService extends HttpService {
  ...
  async changePassword(passwordData) {
    try {
      const response = await this.post("/users/change-password", passwordData);
      return response;
    } catch (error) {
  ...
  async verifyEmail(token) {
    try {
      const response = await this.get(`/users/verify-email?token=${token}`);
      return response;
    } catch (error) {
  ...
  async getUserBookingsByStatus(status, page = 0, size = 10) {
    try {
      const response = await this.get(`/users/bookings/status/${status}`, {
        params: { page, size },
      });
      return response;
    } catch (error) {
  ...
  async getUserBookings(userId = null) {
    try {
      if (userId) {
        const response = await this.get(`/admin/users/${userId}/bookings`);
        return response.data || [];
      } else {
        const response = await this.getUserBookingsByStatus("all", 0, 100);
        return response.data || [];
      }
    } catch (error) {
  ...
  async getUserReviews(userId = null) {
    try {
      if (userId) {
        const response = await this.get(`/admin/users/${userId}/reviews`);
        return response.data || [];
      } else {
        const response = await this.getUserReviewsByStatus("all", 0, 100);
        return response.data || [];
      }
    } catch (error) {
  ...
  async assignRoleToUser(userId, roleName) {
    return await this.post(`/admin/users/${userId}/roles`, { roleName });
  }

  async removeRoleFromUser(userId, roleName) {
    return await this.delete(`/admin/users/${userId}/roles/${roleName}`);
  }
  ...
  async restoreUser(userId) {
    return await this.post(`/admin/users/${userId}/restore`, {});
  }

  async getUserStats() {
    return await this.get("/admin/users/stats");
  }

  async getUserLoginHistory(userId) {
    return await this.get(`/admin/users/${userId}/login-history`);
  }
}
